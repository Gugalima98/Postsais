...
export const updateSheetCell = async (accessToken: string, spreadsheetId: string, rowIndex: number, value: string) => {
    // Writes to Column F (Index 6) at the specific row
    // rowIndex is 0-based index from the data array.
    // Since we started reading at A2, row 0 in data is Row 2 in Sheet.
    const sheetRow = rowIndex + 2; 
    const range = `F${sheetRow}`;
